...
// Patch Notice: This is a patched version of the original `bbcode` plugin. DO NOT EDIT MANUALLY!
...
( function() {
	...
	// Add 'blacklisted' bbcode to mark blacklisted words.
	bbcodeMap['blacklisted'] = 'blacklisted';
	...
	// Disable Smiley-Replacement by a regular expression which shall never match: null
	...
	CKEDITOR.BBCodeParser.prototype = {
		parse: function( bbcode ) {
			...
			while ( ( parts = this._.bbcPartsRegex.exec( bbcode ) ) ) {
				...
				// Tags must not include a space character.
				// Thus, it suffices to take the first word out of the part.
				// This helps with robustness against faulty BBCode where multiple words are parsed as a tag.
				// For example, from [quote author="..."], the tag 'quote author' is parsed.
				part = part.split(" ")[0];
	...
	CKEDITOR.htmlParser.fragment.fromBBCode = function( source ) {
		...
		parser.onTagClose = function( tagName ) {
			...
			while ( candidate.type !== CKEDITOR.NODE_DOCUMENT_FRAGMENT && candidate.name != tagName ) {
			...
			if ( candidate.type !== CKEDITOR.NODE_DOCUMENT_FRAGMENT ) {
	...
	CKEDITOR.plugins.add( 'elasticbbcode', {
		requires: 'entities',
		...
		init: function( editor ) {
			...
			// Disable Paste-From-Word: Only allow plain-text-paste then.
			editor.on('pasteFromWord', function(){ return false; });
			// Configurable Blacklist Function
			if (editor.config.blacklistFn) {
				getTextWithBlacklistHighlighting = editor.config.blacklistFn;
                        }
			...
			bbcodeFilter.addRules( {
				elements: {
					...
					blacklisted: function( element ) {
						element.name = 'span';
						element.attributes.style = "background-color: #FEE7E1; border: 1px solid #F9DCD5;";
						element.attributes.value = element.children[0].value;
					},
					smiley: function( element ) {
						// Remove Smiley, we cannot handle it.
						return false;
						element.name = 'img';
				...
				elements: {
					$: function( element ) {
						...
						if ( tagName in convertMap )
							...
							if ( false && src && src.indexOf( editor.config.smiley_path ) != -1 && alt )
			...
			function onSetData( evt ) {
				...
				var blacklistStartingTag = "[blacklisted]";
				var blacklistEndTag = "[/blacklisted]";
				if (getTextWithBlacklistHighlighting) {
					bbcode = getTextWithBlacklistHighlighting(bbcode, blacklistStartingTag, blacklistEndTag);
				}
